...
var rparser = require('./parser.js')
var util = require('util')
var debug = require('debug')('recurring')
//----------------------------------------------------------------------------------------

function Recurring(promise) {

	var ENDPOINT = 'https://api.recurly.com/v2/'
	var APIKEY
	var AUTH_BASIC
	var parser = rparser.createParser()
	...
	function setAPIKey(key) {
		...
		this.AUTH_BASIC = AUTH_BASIC;
		debug('Setting API Key to %s', APIKEY);
		debug('Setting Authentication header to %s', AUTH_BASIC);
	}

	function setRateLimit(to, per) {
	...
	function execute(options, callback) {
		...
		throttledRequest(options, function (err, response, body) {
			debug('execute got response with status code %s: %j', response.statusCode, body);
			if (err) {
	...
	RecurlyData.go = function (method, uri, args, opts, callback) {
		...
		var options = _.merge(RecurlyData.base_options(), opts);
		options.uri = uri;
		...
		if (promise) {
			return new promise(function (resolve, reject) {
				execute(options, function (err, response, payload) {
					if (err) {
						reject(err);
					} else {
	...
	RecurlyData.prototype.fetch = function (callback) {
		...
		if (!callback) {
			return RecurlyData.get(self.href, {}).then(function (response) {
				if (response.statusCode === 404) throw new Error('not_found');
				self.inflate(response.payload);
			});
		}
	...
	RecurlyData.fetchAll = function (Model, uri, callback) {
		...
		if (!callback) {
			return RecurlyData.get(uri, {per_page: 200}).then(function continuerPromised(response) {
				// link header in response points to next page of results
				// X-Records header says how many total
				if (total < 0)
					total = parseInt(response.headers['x-records'], 10);

				_.each(response.payload, function (record) {
					var item = new Model();
					item.inflate(record);
					result.push(item);
				});
				...
				return RecurlyData.get(uri, {per_page: 200}).then(continuerPromised);
			});
		}
	...
	RecurlyData.createIterator = function (Model, uri) {
		var result = [];
		var total = -1;
		var current = 0;
		...
		return {
			...
			next: function (cb) {
				...
				this.getNextValue(function (err, value) {
					cb(err, value);
				});
			},

			getNextValue: function (done) {
				...
				// Otherwise fetch the next page of results if there are some.
					...
					RecurlyData.get(uri, {per_page: 200}, function (err, response, records) {
	...
	Account.iterator = function () {
		return RecurlyData.createIterator(Account, Account.ENDPOINT);
	};
	...
	BillingInfo.prototype.update = function (options, callback) {
		...
		RecurlyData.put(this.href, body, extraOpts, function (err, response, payload) {
			var error = handleRecurlyError(err, response, payload, [200, 201]);
			if (error)
				return callback(error);
	...
	Coupon.iterator = function () {
		return RecurlyData.createIterator(Coupon, Coupon.ENDPOINT);
	};
	...
	Invoice.iterator = function () {
		return RecurlyData.createIterator(Invoice, Invoice.ENDPOINT);
	};
	...
	Plan.iterator = function () {
		return RecurlyData.createIterator(Plan, Plan.ENDPOINT);
	};
	...
	Subscription.iterator = function () {
		return RecurlyData.createIterator(Subscription, Subscription.ENDPOINT);
	};
	...
	Transaction.all = function (callback) {
		if (Transaction.__all)
			return callback(Transaction.__all);

		RecurlyData.fetchAll(Transaction, Transaction.ENDPOINT, function (err, results) {
			Transaction.__all = {};
			_.each(results, function (transaction) {
				Transaction.__all[transaction.uuid] = transaction;
			});
			callback(Transaction.__all);
		});
	};
	...
	Transaction.iterator = function () {
		return RecurlyData.createIterator(Transaction, Transaction.ENDPOINT);
	};
	...
	Recurring.prototype.setRateLimit = setRateLimit;

}

module.exports = function (promise) {
	return new Recurring(promise);
};
